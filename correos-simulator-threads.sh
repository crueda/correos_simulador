	...
	if (len(sys.argv) == 3):
		...
		for imei in imeis:
			try:
				...
				print "Error connecting for imei %s" % imei
			
		for position in positions:
			for imei in imeis:
				date = time.strftime("%Y%m%d%H%M%S")
				random = float(randint(1000, 9999))/100000
				lat = str(float (position.split(",")[1]))
				lon = str(float (position.split(",")[0]))
				frame = FRAME.replace("imei",imei).replace("date",date).replace("lon",lon).replace("lat",lat)
				connection[imei].send(frame + "\r\n")
				logger.info('[%s] sent: %s' , imei, frame)
				print '[%s] sent: %s' % (imei, frame)
				#time.sleep(0.015)
			time.sleep(FREQUENCY)
